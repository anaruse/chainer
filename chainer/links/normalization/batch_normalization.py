...
class BatchNormalization(link.Link):
    ...
    def __init__(self, size, decay=0.9, eps=2e-5, dtype=numpy.float32,
                 ...
                 initial_gamma=None, initial_beta=None, key_axis=None):
    ...
    def __call__(self, x, **kwargs):
        ...
        if configuration.config.train:
            ...
            ret = functions.batch_normalization(
                ...
                running_var=self.avg_var, decay=decay, key_axis=self.key_axis)
        else:
            # Use running average statistics or fine-tuned statistics.
            mean = variable.Variable(self.avg_mean)
            var = variable.Variable(self.avg_var)
            ret = functions.fixed_batch_normalization(
                x, gamma, beta, mean, var, self.eps, key_axis=self.key_axis)
        return ret
